...
class OpprydningIntegrationTest : FellesTestOppsett() {
    ...
    @Autowired
    lateinit var opprydningJobb: OpprydningJobb
    ...
    @Test
    @Order(3)
    fun `Vi kjører opprydning og Donner beskjed på Ditt NAV og melding på Ditt Sykefravær`() {
        statusRepository
            .hentAlleMedNyesteStatus(StatusVerdi.DITT_SYKEFRAVAER_MANGLER_INNTEKTSMELDING_SENDT).shouldHaveSize(1)

        opprydningJobb.jobMedParameter()
        ...
        statusHistorikk shouldHaveSize 5

        statusRepository
            .hentAlleMedNyesteStatus(StatusVerdi.DITT_SYKEFRAVAER_MANGLER_INNTEKTSMELDING_SENDT).shouldHaveSize(0)
    }
